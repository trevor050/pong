...
function paintBall(ctxTarget, ball, palette, style, radius) {
  ...
  if (style === "fill") {
    ...
    // Double ring with soft glow
    const glow = ctxTarget.createRadialGradient(0, 0, radius * 0.5, 0, 0, radius * 1.6);
    glow.addColorStop(0, rgba(palette.rim, 0.0));
    glow.addColorStop(0.5, rgba(palette.rim, 0.25));
    glow.addColorStop(1, "rgba(0,0,0,0)");
    ctxTarget.fillStyle = glow;
    ctxTarget.beginPath();
    ctxTarget.arc(0, 0, radius * 1.6, 0, Math.PI * 2);
    ctxTarget.fill();

    ctxTarget.strokeStyle = rgba(palette.rim, 0.9);
    ctxTarget.lineWidth = Math.max(1.5, radius * 0.15);
    ctxTarget.beginPath();
    ctxTarget.arc(0, 0, radius * 1.0, 0, Math.PI * 2);
    ctxTarget.stroke();

    ctxTarget.strokeStyle = rgba(palette.outline, 0.6);
    ctxTarget.lineWidth = Math.max(1, radius * 0.1);
    ctxTarget.beginPath();
    ctxTarget.arc(0, 0, radius * 0.6, 0, Math.PI * 2);
    ctxTarget.stroke();
  } else if (style === "pixel") {
    ctxTarget.fillStyle = rgba(palette.rim, 1);
    const s = radius * 1.6;
    ctxTarget.fillRect(-s/2, -s/2, s, s);
    
    ctxTarget.fillStyle = rgba(palette.outline, 0.4);
    ctxTarget.fillRect(-s/2 + 2, -s/2 + 2, s - 4, s - 4);
    
    ctxTarget.fillStyle = rgba("#ffffff", 0.8);
    ctxTarget.fillRect(-s/4, -s/4, s/2, s/2);

  } else if (style === "shatter") {
    ctxTarget.fillStyle = rgba(palette.rim, 0.8);
    ctxTarget.beginPath();
    ctxTarget.moveTo(-radius, 0);
    ctxTarget.lineTo(-radius * 0.3, -radius * 0.3);
    ctxTarget.lineTo(0, -radius);
    ctxTarget.lineTo(radius * 0.3, -radius * 0.3);
    ctxTarget.lineTo(radius, 0);
    ctxTarget.lineTo(radius * 0.3, radius * 0.3);
    ctxTarget.lineTo(0, radius);
    ctxTarget.lineTo(-radius * 0.3, radius * 0.3);
    ctxTarget.fill();
    ...
    // Highlight
    ctxTarget.fillStyle = rgba("#ffffff", 0.6);
    ctxTarget.beginPath();
    ctxTarget.arc(-radius * 0.4, -radius * 0.4, radius * 0.25, 0, Math.PI * 2);
    ctxTarget.fill();
  } else if (style === "grid") {
    ctxTarget.save();
    ctxTarget.beginPath();
