...
function updateDynamicBackground(dayRatio = 0.5, topHalfRatio = dayRatio, centroid = { x: 0.5, y: 0.5 }) {
  if (isImmersive) {
    document.body.style.background = "#000"; // Black out background in fullscreen
    return;
  }
  if (!backgroundSeed) refreshBackgroundSeed();
  
  // Ambilight effect: Extend light from edges
  const dayColor = colors.day;
  const nightColor = colors.night;
  const accentColor = colors.accent;

  // Calculate dominance for "light of the ends"
  const dayIntensity = dayRatio;
  const nightIntensity = 1 - dayRatio;

  const bg = `
    radial-gradient(circle at 0% 50%, ${rgba(dayColor, 0.4 * dayIntensity)}, transparent 50%),
    radial-gradient(circle at 100% 50%, ${rgba(nightColor, 0.4 * nightIntensity)}, transparent 50%),
    radial-gradient(circle at 50% 0%, ${rgba(dayColor, 0.2 * topHalfRatio)}, transparent 50%),
    radial-gradient(circle at 50% 100%, ${rgba(nightColor, 0.2 * (1 - topHalfRatio))}, transparent 50%),
    linear-gradient(135deg, #050911 0%, #0c1526 100%)
  `;
