...
function drawBall(ball) {
  ...
  if (settings.ballStyle === "solid") {
    ...
    // Neon Glow (default)
    const contrastColor = ball.owner === 0 ? colors.night : "#ffffff";
    
    // Outer glow
    const glow = ctx.createRadialGradient(0, 0, ballRadius * 0.3, 0, 0, ballRadius * 1.3);
    glow.addColorStop(0, rgba(accent, 0.6));
    glow.addColorStop(0.5, rgba(accent, 0.25));
    glow.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = glow;
    ctx.beginPath();
    ctx.arc(0, 0, ballRadius * 1.3, 0, Math.PI * 2);
    ctx.fill();
    
    // Main ring with contrast
    ...
    ringGrad.addColorStop(1, rgba(accent, 0.9));
    ctx.lineWidth = Math.max(2.8, ballRadius * 0.42);
    ctx.strokeStyle = ringGrad;
    ctx.beginPath();
    ctx.arc(0, 0, ballRadius * 0.85, 0, Math.PI * 2);
    ctx.stroke();
    ctx.shadowBlur = 0;
  }
  ctx.restore();
...
function initControls() {
  ...
  fullscreenHint.addEventListener("click", () => toggleBrowserFullscreen());
  drawerFullscreen.addEventListener("click", () => toggleBrowserFullscreen());
