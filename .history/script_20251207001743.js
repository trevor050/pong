...
function updateDynamicBackground(dayRatio = 0.5, topHalfRatio = dayRatio, centroid = { x: 0.5, y: 0.5 }) {
  ...
  // "Shooting out rays" effect
  // We want the dominant color to project outwards.
  // If day is dominant (dayRatio > 0.5), day color projects.
  
  const isDayDominant = dayRatio > 0.5;
  const dominantColor = isDayDominant ? dayColor : nightColor;
  const secondaryColor = isDayDominant ? nightColor : dayColor;
  const intensity = Math.abs(dayRatio - 0.5) * 2; // 0 to 1 based on dominance
  
  // Create a radial gradient that feels like it's projecting light
  // The center is the game area (transparent), and it shoots out.
  
  const bg = `
    radial-gradient(circle at 50% 50%, transparent 0%, transparent 30%, ${rgba(dominantColor, 0.1 + intensity * 0.4)} 60%, ${rgba(dominantColor, 0.6 + intensity * 0.4)} 100%),
    radial-gradient(circle at 50% 50%, ${rgba(secondaryColor, 0.1)} 0%, ${rgba(dominantColor, 0.05)} 100%),
    linear-gradient(180deg, #050911 0%, #0c1526 100%)
  `;
...
function updateFullscreenHint() {
  // Only show if stretched "a bunch" (e.g., > 90% of screen)
...
function enterFullscreen() {
  ...
  // Fullscreen means FULL screen, no margins
...
function exitFullscreen() {
  ...
  // Restore to a safe size, ensuring notch doesn't go off screen
  const safeH = window.innerHeight - 100; // Space for notch
  const targetW = savedSize ? Math.min(savedSize.w, window.innerWidth - 48) : Math.min(window.innerWidth - 120, 900);
  const targetH = savedSize ? Math.min(savedSize.h, safeH) : Math.min(window.innerHeight - 160, 900);
...
function setFrameSize(width, height, preserveOwnership = true) {
  ...
  const marginH = isImmersive ? 0 : 100; // More bottom margin for notch
  const maxW = isImmersive ? window.innerWidth : Math.max(420, window.innerWidth - marginW);
  const maxH = isImmersive ? window.innerHeight : Math.max(420, window.innerHeight - marginH);
...
function drawBall(ball) {
  ...
  if (settings.ballStyle === "solid") {
    ...
    ctx.stroke();
  } else if (settings.ballStyle === "design") {
    // Cool Design: Geometric pattern
    ctx.save();
    ctx.beginPath();
  ...
  } else if (settings.ballStyle === "artsy") {
    // Artsy Ghost: Trail/Ghost effect
    // Draw ghost
    ctx.fillStyle = rgba(baseColor, 0.3);
    ctx.beginPath();
    ctx.arc(-ball.vx * 2, -ball.vy * 2, ballRadius * 0.8, 0, Math.PI * 2);
    ctx.fill();
    
    ctx.fillStyle = rgba(baseColor, 0.6);
    ctx.beginPath();
    ctx.arc(-ball.vx, -ball.vy, ballRadius * 0.9, 0, Math.PI * 2);
    ctx.fill();

    // Main ball
    const grad = ctx.createRadialGradient(-ballRadius * 0.2, -ballRadius * 0.2, 0, 0, 0, ballRadius);
    grad.addColorStop(0, rgba(baseColor, 1));
    grad.addColorStop(1, rgba(baseColor, 0.4));
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.arc(0, 0, ballRadius, 0, Math.PI * 2);
    ctx.fill();
    ...
    // Neon Ring (Simple Ring)
    // Just a clean stroke, no shadow/glow
    ctx.shadowBlur = 0;
    ctx.lineWidth = Math.max(2, ballRadius * 0.25);
    
    // For Day ball (owner 0), ensure contrast
    let strokeColor = baseColor;
    if (ball.owner === 0) {
        // If it's day, use a slightly darker/stronger version or the accent if needed
        // But user wants "contrast the background better".
        // Let's use a mix of the base and a bit of black/ink for definition
        strokeColor = mixColors(baseColor, "#000000", 0.2);
    }
