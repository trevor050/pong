...
const roundShapesInput = document.querySelector("#roundShapesInput");
...
const themes = {
  ...
  midnightCity: { label: "Midnight City", day: "#e0e7ff", night: "#1a1b4b", accent: "#f472b6" },
  forestFire: { label: "Forest Fire", day: "#ecfdf5", night: "#064e3b", accent: "#f59e0b" },
  oceanBreeze: { label: "Ocean Breeze", day: "#f0f9ff", night: "#0c4a6e", accent: "#38bdf8" },
  lavenderDream: { label: "Lavender Dream", day: "#faf5ff", night: "#3b0764", accent: "#d8b4fe" }
};
...
function updateDynamicBackground(dayRatio = 0.5, topHalfRatio = dayRatio) {
  ...
  // Made accent glow much more subtle and larger
  const accentGlow = rgba(colors.accent, 0.08); 
  document.body.style.background = `radial-gradient(circle at 20% 20%, ${rgba(
    colors.day,
    0.05
  )}, transparent 60%), radial-gradient(circle at 80% 80%, ${accentGlow}, transparent 60%), linear-gradient(160deg, ${top} 0%, ${mid} 50%, ${bottom} 100%)`;
}
...
function nonLinearSpeed(raw) {
  // More resistive / exponential curve
  // 0 -> 0.1x
  // 50 -> ~1.3x
  // 100 -> 10x
  const min = 0.1;
  const max = 10.0;
  const t = Math.pow(raw / 100, 3); 
...
function updateDerived() {
  ...
  // Adjust max speed cap to allow for the higher multiplier
  if (settings.speedMood > 3) {
      maxSpeed = 20 * settings.speedMood; 
  }
}
...
function drawTile(tile, owner) {
  ...
  if (settings.roundShapes) {
    ctx.beginPath();
    ctx.arc(tile.x + settings.tileSize / 2, tile.y + settings.tileSize / 2, settings.tileSize / 2, 0, Math.PI * 2);
    ctx.fill();
  } else {
...
function initControls() {
  ...
  roundShapesInput.addEventListener("change", (e) => {
    settings.roundShapes = e.target.checked;
  });

  themeSelect.addEventListener("change", (e) => {target.value));
